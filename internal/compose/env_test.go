...
func TestGenerateEnvExample_NoSensitiveValues(t *testing.T) {
	g := newTestGenerator(t)
	// pgsql: required to emit DB_PASSWORD and DB_URL as active keys
	//        (DB_URL is a pgsql-only extra not in the base template).
	// redis: included for completeness (REDIS_PASSWORD is active in base template regardless).
	cfg := &config.Config{
		PHP:      config.PHP{Version: "8.5", Runtime: "frankenphp"},
	...
	// Each sensitive key must be present with an empty value.
