...
func generate(cfg *config.Config, dir, version string) error {
	...
	ephemeralPorts := config.IsWorktree(dir)
	vitePort := 5173
	if ephemeralPorts {
	...
	viteServer := buildViteServerJS(cfg, vitePort)
...
func buildViteServerJS(cfg *config.Config, vitePort int) string {
	if !cfg.Server.IsHTTPS() {
		return "// Generated by Frank — Vite dev server config\nexport default {\n    host: '0.0.0.0',\n};\n"
	}
	return fmt.Sprintf(`// Generated by Frank — Vite dev server config
import fs from 'fs';
